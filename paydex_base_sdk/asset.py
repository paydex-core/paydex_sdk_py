...
class Asset:
    ...
    corresponding issuer on the Paydex network.

    For more information about the formats used for asset codes and how issuers
    work on Paydex's network, see `Paydex's guide on assets`_.

    :param code: The asset code, in the formats specified in `Paydex's
        guide on assets`_.
    :param issuer: The account ID of the issuer. Note if the
        currency is the native currency (PAYDEX (Lumens)), no issuer is necessary.
    :raises:
        | :exc:`AssetCodeInvalidError <paydex_sdk.exceptions.AssetCodeInvalidError>`: if ``code`` is invalid.
        | :exc:`AssetIssuerInvalidError <paydex_sdk.exceptions.AssetIssuerInvalidError>`: if ``issuer`` is not a valid ed25519 public key.
    ...
    def __init__(self, code: str, issuer: Optional[str] = None) -> None:
        ...
        if code != "PAYDEX" and issuer is None:
    ...
    def guess_asset_type(self) -> str:
        ...
        :return: The type of the asset.
        """
        if self.code == "PAYDEX" and self.issuer is None:
            asset_type = "native"
