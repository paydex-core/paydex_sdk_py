"""
SEP: 0005
Title: Key Derivation Methods for Paydex Accounts
"""
...
class PaydexMnemonic(Mnemonic):
    ...
    def derive(seed: bytes, index: int) -> bytes:
        ...
        master_hmac = hmac.new(PaydexMnemonic.SEED_MODIFIER, digestmod=hashlib.sha512)
        master_hmac.update(seed)
        il = master_hmac.digest()[:32]
        ir = master_hmac.digest()[32:]
        path = PaydexMnemonic.PAYDEX_ACCOUNT_PATH_FORMAT % index
        for x in path.split("/")[1:]:
            data = (
                struct.pack("x")
                + il
                + struct.pack(">I", PaydexMnemonic.FIRST_HARDENED_INDEX + int(x[:-1]))
            )
